...
/// @title MarketFactory — registers verified creators and deploys binary FPMM markets on Arc.
/// @notice `registerCreator` is called by the backend (verifier) AFTER a World ID 4.0 check —
/// no ENS params (ENS removed in Variant A). Markets accept USDC or EURC collateral.
contract MarketFactory {
    ...
    function createMarket(
        ...
        require(collateral.transferFrom(msg.sender, address(m), initialLiquidity), "xferL");
